...
class Process:
    def __init__(self, apid, tm_distributor, scheduler, housekeeping=False, event_reporting=False, function_management=False, test=False):
        ...
        self._pus_services = {}
        self._pus_service_1 = services.RequestVerification(self._ident, tm_distributor)
        self._pus_services[1] = self._pus_service_1
        if housekeeping:
            self._pus_service_3 = services.Housekeeping(self._ident, self._pus_service_1, tm_distributor)
            self._pus_services[3] = self._pus_service_3
        if event_reporting:
            self._pus_service_5 = services.EventReporting(self._ident, self._pus_service_1, tm_distributor)
