...
class FileInput(InputStream):
    ...
    def __iter__(self):
        ...
        while offset < len(data):
            ...
            packet_length, packet = get_pus_policy().deserialize_tc(data[offset:], validate_fields=False, validate_pec=self._validate_pec)
    ...
    def read(self, offset=0):
        with open(self._input, 'rb') as f:
            content = f.read()
        data = memoryview(content)
        _, packet = get_pus_policy().deserialize_tc(data[self._other_headers_size + offset:], validate_fields=False, validate_pec=False)
        return packet
