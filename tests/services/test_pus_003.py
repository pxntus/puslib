...
def test_structure_report(service_3_setup, is_diagnostic_report):
    ...
    fmt = '>' + f"{get_pus_policy().housekeeping.structure_id_type().format}{get_pus_policy().housekeeping.collection_interval_type().format}{get_pus_policy().housekeeping.count_type().format}".replace('>', '')
    sid, collection_interval, num_params = struct.unpack(fmt, tm.source_data[:struct.calcsize(fmt)])
    assert sid == 1
    assert collection_interval == 1000
    assert num_params == 3
    fmt2 = '>' + f"{num_params}{get_pus_policy().common.param_id_type().format}".replace('>', '')
...
def test_parameter_report(service_3_setup, is_diagnostic_report):
    ...
    fmt = f"{get_pus_policy().housekeeping.structure_id_type().format}"
