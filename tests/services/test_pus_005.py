...
def test_manual_event(service_5_setup):
    ...
    assert len(report.source_data) == get_pus_policy().event_reporting.event_definition_id_type().size
    assert get_pus_policy().event_reporting.event_definition_id_type.from_bytes(report.source_data) == 0
...
def test_event_with_data(service_5_setup):
    ...
    assert tm_stream.size == 1
    report = tm_stream.get()
    id_param = get_pus_policy().event_reporting.event_definition_id_type()
...
def test_trigger_events(service_5_setup):
    ...
    assert report.service == 5
    assert report.subservice == Severity.INFO
    assert get_pus_policy().event_reporting.event_definition_id_type.from_bytes(report.source_data) == 3
    ev.disable()
    param1.value += 1
    assert tm_stream.size == 0
    ...
    assert get_pus_policy().event_reporting.event_definition_id_type.from_bytes(report.source_data) == 5
    ...
    assert get_pus_policy().event_reporting.event_definition_id_type.from_bytes(report.source_data) == 7
...
def test_toggle_event(service_5_setup):
    ...
    app_data = get_pus_policy().event_reporting.count_type(1).to_bytes() + get_pus_policy().event_reporting.event_definition_id_type(0).to_bytes()
    ...
    app_data = get_pus_policy().event_reporting.count_type(1).to_bytes() + get_pus_policy().event_reporting.event_definition_id_type(1).to_bytes()
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=5, service_subtype=6, data=app_data)
    pus_service_5.enqueue(packet)
    pus_service_5.process()
    assert not ev2.enabled

    app_data = get_pus_policy().event_reporting.count_type(2).to_bytes() + get_pus_policy().event_reporting.event_definition_id_type(2).to_bytes() + get_pus_policy().event_reporting.event_definition_id_type(3).to_bytes()
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=5, service_subtype=5, data=app_data)
    pus_service_5.enqueue(packet)
    pus_service_5.process()
    assert ev3.enabled
...
def test_disabled_event_report(service_5_setup):
    ...
    assert len(report.source_data) == get_pus_policy().event_reporting.count_type().size
    assert get_pus_policy().event_reporting.count_type.from_bytes(report.source_data) == 0
    ...
    assert report.service == 5
    assert report.subservice == 8
    assert len(report.source_data) == get_pus_policy().event_reporting.count_type().size + get_pus_policy().event_reporting.event_definition_id_type().size * 2
    fmt = '>' + f"{get_pus_policy().event_reporting.count_type().format}2{get_pus_policy().event_reporting.event_definition_id_type().format}".replace('>', '')
    ...
    assert report.service == 5
    assert report.subservice == 8
    assert len(report.source_data) == get_pus_policy().event_reporting.count_type().size + get_pus_policy().event_reporting.event_definition_id_type().size * 4
    fmt = '>' + f"{get_pus_policy().event_reporting.count_type().format}4{get_pus_policy().event_reporting.event_definition_id_type().format}".replace('>', '')
