...
def test_report_parameter_values(service_20_setup):
    ...
    app_data = bytes(get_policy().function_management.count_type(1)) + bytes(get_policy().common.param_id_type(0))
    ...
    app_data = bytes(get_policy().function_management.count_type(2)) + bytes(get_policy().common.param_id_type(0)) + bytes(get_policy().common.param_id_type(3))
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
    ...
    app_data = bytes(get_policy().function_management.count_type(1)) + bytes(get_policy().common.param_id_type(1))  # non-existant parameter ID
    ...
    app_data = bytes(get_policy().function_management.count_type(2)) + bytes(get_policy().common.param_id_type(0)) + bytes(get_policy().common.param_id_type(1))  # non-existant parameter ID
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
    ...
    app_data = bytes(get_policy().function_management.count_type(3)) + bytes(get_policy().common.param_id_type(0)) + bytes(get_policy().common.param_id_type(1))  # mismatch between N and number of parameter IDs
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
...
def test_set_parameter_values(service_20_setup):
    ...
    app_data = bytes(get_policy().function_management.count_type(1)) + bytes(get_policy().common.param_id_type(0)) + bytes(UInt32Parameter(11))
    ...
    app_data = bytes(get_policy().function_management.count_type(2)) + bytes(get_policy().common.param_id_type(3)) + bytes(Int16Parameter(-12)) + bytes(get_policy().common.param_id_type(5)) + bytes(Real64Parameter(13.0))
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=20, service_subtype=3, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 0
    ...
    app_data = bytes(get_policy().function_management.count_type(1)) + bytes(get_policy().common.param_id_type(1)) + bytes(UInt32Parameter(11))  # non-existant parameter ID
    ...
    app_data = bytes(get_policy().function_management.count_type(1)) + bytes(get_policy().common.param_id_type(1)) + bytes(UInt32Parameter(1)) + bytes(get_policy().common.param_id_type(2)) + bytes(UInt32Parameter(666))  # non-existant parameter ID
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=3, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
