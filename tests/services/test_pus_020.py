...
def test_report_parameter_values(service_20_setup):
    ...
    fmt = '>' + (get_pus_policy().NType().format + get_pus_policy().IdType().format + UInt32Parameter().format).replace('>', '')
    num_values, param_id, param_value = struct.unpack(fmt, report.source_data)
    assert num_values == 1
    assert param_id == 0
    assert param_value == 1

    app_data = get_pus_policy().NType(2).to_bytes() + get_pus_policy().IdType(0).to_bytes() + get_pus_policy().IdType(3).to_bytes()
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
    ...
    assert report.subservice == 2
    assert report.source_data is not None
    fmt = '>' + (get_pus_policy().NType().format + get_pus_policy().IdType().format + UInt32Parameter().format + get_pus_policy().IdType().format + Int16Parameter().format).replace('>', '')
    ...
    app_data = get_pus_policy().NType(1).to_bytes() + get_pus_policy().IdType(1).to_bytes()  # non-existant parameter ID
    ...
    app_data = get_pus_policy().NType(2).to_bytes() + get_pus_policy().IdType(0).to_bytes() + get_pus_policy().IdType(1).to_bytes()  # non-existant parameter ID
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
    ...
    app_data = get_pus_policy().NType(3).to_bytes() + get_pus_policy().IdType(0).to_bytes() + get_pus_policy().IdType(1).to_bytes()  # mismatch between N and number of parameter IDs
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=1, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
...
def test_set_parameter_values(service_20_setup):
    ...
    app_data = get_pus_policy().NType(1).to_bytes() + get_pus_policy().IdType(0).to_bytes() + UInt32Parameter(11).to_bytes()
    ...
    app_data = get_pus_policy().NType(2).to_bytes() + get_pus_policy().IdType(3).to_bytes() + Int16Parameter(-12).to_bytes() + get_pus_policy().IdType(5).to_bytes() + Real64Parameter(13.0).to_bytes()
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.NONE, service_type=20, service_subtype=3, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 0
    ...
    app_data = get_pus_policy().NType(1).to_bytes() + get_pus_policy().IdType(1).to_bytes() + UInt32Parameter(11).to_bytes()  # non-existant parameter ID
    ...
    app_data = get_pus_policy().NType(1).to_bytes() + get_pus_policy().IdType(1).to_bytes() + UInt32Parameter(1).to_bytes() + get_pus_policy().IdType(2).to_bytes() + UInt32Parameter(666).to_bytes()  # non-existant parameter ID
    packet = PusTcPacket.create(apid=ident.apid, name=0, ack_flags=AckFlag.ACCEPTANCE, service_type=20, service_subtype=3, data=app_data)
    pus_service_20.enqueue(packet)
    pus_service_20.process()
    assert tm_stream.size == 1
